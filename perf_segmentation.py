...
mono_files = sorted(glob(join(config.out_dir, "original_mono_round", "*.lab")))
full_files = sorted(glob(join(config.out_dir, "original_full_round", "*.lab")))
dst_dir = join(config.out_dir, "full_aligned")
...
# segmentation
base_files = sorted(glob(join(config.out_dir, "original_mono_round", "*.lab")))

lengths = {}

for name in ["full_aligned", "original_full_round"]:
    ...
    for idx, base in enumerate(base_files):
        ...
        base_dst_dir = join(config.out_dir, "mono_seg")
